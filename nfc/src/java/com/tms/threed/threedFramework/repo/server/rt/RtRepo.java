...
import com.tms.threed.threedFramework.imageModel.server.BlinkChecker;
...
public class RtRepo implements BlinkChecker {
    ...
    public RtRepo(File rtRepoDir, SeriesKey seriesKey) {
        ...
        RepoUtil.createDirNotExists(rtRepoDir);

        this.modelsDir = new File(rtRepoDir, "models");
        RepoUtil.createDirNotExists(modelsDir);

        this.commitsDir = new File(rtRepoDir, "commits");
        RepoUtil.createDirNotExists(commitsDir);

        this.jpgDir = new File(rtRepoDir, "jpgs");
        RepoUtil.createDirNotExists(jpgDir);

        this.pngInfoDir = new File(rtRepoDir, "png-info");
        RepoUtil.createDirNotExists(pngInfoDir);

        emptyPngDir = new File(pngInfoDir, "empty");
        RepoUtil.createDirNotExists(emptyPngDir);

        notEmptyPngDir = new File(pngInfoDir, "not-empty");
        RepoUtil.createDirNotExists(notEmptyPngDir);

        versionsDir = new File(this.rtRepoDir, "versions");
        RepoUtil.createDirNotExists(versionsDir);


        blinksDir = new File(this.rtRepoDir, "blinks");
        RepoUtil.createDirNotExists(blinksDir);
...
//                    }
    ...
    public File getJpgFileName(JpgId jpgId) {
        File jpgDirForSize = getJpgDirForSize(jpgId.getWidth());
        JpgId.TwoThirty8 twoThirty8 = jpgId.getTwoThirty8();
        File jpgFileName = twoThirty8.getFileName(jpgDirForSize);
        return jpgFileName;
    }

    public boolean exists(JpgId jpgId) {
        File f = getJpgFileName(jpgId);
        return f.exists();
    }
